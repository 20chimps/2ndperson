...
#else
					Undo.RecordObject(fracturedChunk.renderer, "Assign Inside Material");
#endif

                    if(fracturedChunk.SplitSubMeshIndex != -1)
                    {
                        Material[] aMaterials = new Material[fracturedChunk.renderer.sharedMaterials.Length];

                        for(int nMaterial = 0; nMaterial < aMaterials.Length; nMaterial++)
                        {
                            aMaterials[nMaterial] = nMaterial == fracturedChunk.SplitSubMeshIndex ? PropSplitMaterial.objectReferenceValue as Material : fracturedChunk.renderer.sharedMaterials[nMaterial];
                        }

                        fracturedChunk.renderer.sharedMaterials = aMaterials;
                    }
                }
            }
        }
