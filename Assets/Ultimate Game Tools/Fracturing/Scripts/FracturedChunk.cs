...
public class FracturedChunk : MonoBehaviour
{
    ...
    void Update()
    {
        if(Application.isPlaying)
        {
            if(renderer.isVisible == false && IsDetachedChunk == true)
            {
    ...
    {
        if(rigidbody != null && IsDestructibleChunk())
        {
            List<FracturedChunk> listBreaks = new List<FracturedChunk>();
            ...
            if(IsDetachedChunk == false)
            {
                ...
                {
                    chunk.DetachFromObject();
                    chunk.rigidbody.AddExplosionForce(fExplosionForce, v3Position, 0.0f, 0.0f);
                }
            }
            ...
            {
                chunk.DetachFromObject();
                chunk.rigidbody.AddExplosionForce(fExplosionForce, v3Position, 0.0f, FracturedObjectSource.EventDetachUpwardsModifier);
            }
        }
    ...
    {
        if(IsDestructibleChunk() && IsDetachedChunk == false && rigidbody)
        {
            m_bNonSupportedChunkStored = IsNonSupportedChunk;
